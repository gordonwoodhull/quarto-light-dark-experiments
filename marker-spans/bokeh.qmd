...
This example is not working because
* Bokeh has multiple HTML outputs (`.cell-output-display`) per plot.
* Properties for custom themes don't seem to be fully documented.
* Claude's implementation of a violin plot is comically wrong, if recognizable.
...
set_brand_theme('united-brand.yml')(p)
show(p)

display(Markdown('[]{.quarto-light-marker}'))
set_brand_theme('slate-brand.yml')(p)
show(p)
# print(p._repr_html_())

# print([x for x in dir(p) if re.match('.*repr.*', x)])
# LightDarkStateful(
